...
struct Node {
    pll p = {LONG_LONG_MAX,LONG_LONG_MIN};    
};
...
int main()
{
    ...
    // while(t--){
        ll l=0,r=1,n,k,mid=0,ans=0,m,i=0,j=0,c0=0,c1=0,x=0,y=-1,z=LONG_LONG_MIN,MOD=1e9 + 7,a=0,b=0,c=0,d=0,e=0,f=0,g;                
        ...
        while(i<j){
            y=v2[i].first+v2[j].first;
            if(y>x){
                j--;
            }else if(y<x){
                i++;
            }else{
                f=1;a=v2[i].second+1,b=v2[j].second+1;break;
            }
        }
