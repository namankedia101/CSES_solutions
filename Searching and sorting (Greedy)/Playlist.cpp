...
int main()
{
    ...
    cin.tie(NULL);
        ...
        ll l=-1,r=2e9,n,k,mid=0,ans=1,m,i=0,j=0,c0=0,c1=0,x=0,y=-1,z=LONG_LONG_MIN,MOD=1e9 + 7,a=0,b=0,c=0,d=0,e=0,f=0,g;                
        ...
        while(j<n){
            if(i==j){mp[v[i]]=1;j++;continue;}
            if(mp[v[j]]==1){
                while(v[i]!=v[j]){mp[v[i]]=0;i++;}
                mp[v[i]]=0;
                i++;
            }else{ans=max(ans,j-i+1);mp[v[j]]=1;j++;}
        }
